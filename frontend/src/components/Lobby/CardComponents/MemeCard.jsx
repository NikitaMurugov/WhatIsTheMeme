...
const MemeCard = ({card}) => {
    return <>
        <div
            className={`h-28 w-56 border-4 border-white ${card.active 
        ...
        >
            ...
            {!card.img && card.active ? <BiImage color={"white"} className={"w-10 h-10"}/> : <BiImage color={"rgb(107 114 128 / 1"} className={"w-7 h-7 "}/> }
        </div>
    </>
}
